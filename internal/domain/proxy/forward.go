...
func (p *WAFProxy) detectResponseBodyContent(resp *http.Response, bodyCopy []byte) {
	...
	requestID := getRequestID(resp.Request)
	...
	if len(observeResults) > 0 {
		...
		log := logger.NewAccessLog().
			...
			SetMethod(resp.Request.Method).
			SetPath(masker.MaskPath(resp.Request.URL.Path)).
			SetStatus(http.StatusOK).
			SetAction("observe").
			SetRequestID(requestID).
			SetRuleID("观察模式:" + obsType).
			SetHost(resp.Request.Host).
			SetQuery(masker.MaskQuery(resp.Request.URL.RawQuery)).
			SetUserAgent(resp.Request.Header.Get("User-Agent")).
			SetReferer(resp.Request.Header.Get("Referer")).
			SetContentType(resp.Request.Header.Get("Content-Type")).
...
func (p *WAFProxy) detectSensitiveData(resp *http.Response, bodyCopy []byte) {
	...
	if len(blockResults) > 0 {
		attackTypes := p.detectorManager.GetAttackTypes(blockResults)
...
func (p *WAFProxy) detectDLPInResponse(resp *http.Response, bodyCopy []byte) {
	...
	isObserve := p.detectorManager != nil && p.detectorManager.IsObservationMode("sensitive_data")
	...
	if isObserve {
		obsDetail := strings.Join(dlpDetails, ", ")
		log := logger.NewAccessLog().
			SetClientIP(cIP).
			SetMethod(resp.Request.Method).
			SetPath(masker.MaskPath(resp.Request.URL.Path)).
