...
func (cm *ConfigManager) initBuiltinRules() error {
	// SQL注入内置规则
	var count int
	err := cm.db.QueryRow("SELECT COUNT(*) FROM detection_rules WHERE detector_type='sql_injection' AND rule_type='builtin'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
	...
	// XSS内置规则
	err = cm.db.QueryRow("SELECT COUNT(*) FROM detection_rules WHERE detector_type='xss' AND rule_type='builtin'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
	...
	// 命令注入内置规则
	err = cm.db.QueryRow("SELECT COUNT(*) FROM detection_rules WHERE detector_type='command_injection' AND rule_type='builtin'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
	...
	// SSRF内置规则
	err = cm.db.QueryRow("SELECT COUNT(*) FROM detection_rules WHERE detector_type='ssrf' AND rule_type='builtin'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
	...
	// XXE内置规则
	err = cm.db.QueryRow("SELECT COUNT(*) FROM detection_rules WHERE detector_type='xxe' AND rule_type='builtin'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
	...
	// NoSQL内置规则
	err = cm.db.QueryRow("SELECT COUNT(*) FROM detection_rules WHERE detector_type='nosql' AND rule_type='builtin'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
	...
	// SSTI内置规则
	err = cm.db.QueryRow("SELECT COUNT(*) FROM detection_rules WHERE detector_type='ssti' AND rule_type='builtin'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
